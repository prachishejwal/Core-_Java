...
public class ThreadUnsafeCollections {
	public static void main(String[] args) throws Exception {
		...
		IntStream.range(1, 10001).forEach(list::add);//i->list.add(i);
		...
		Thread writer = new Thread(() -> {
			synchronized (list) {
				IntStream.range(10001, 20001).forEach(list::add);//thats why showing concurrent modification due modify in list
			}
